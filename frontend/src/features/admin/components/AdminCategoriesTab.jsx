  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 relative pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <button onClick={() => openModal()} className="px-4 py-2 bg-fg text-bg rounded-xl font-medium text-sm flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"><Plus size={16} /> Нова категорія</button>
      </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan="4" className="px-4 py-16 text-center text-muted">
                    <FolderTree size={32} className="mx-auto mb-3 opacity-20" /><p>Категорій не знайдено.</p>
                  </td>
                </tr>
              ) : (
                categories.map((cat) => {
                  ...
                  return (
                    <tr key={cat._id || cat.id} className="hover:bg-fg/5 transition-colors group">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => toggleActiveStatus(cat)} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${isActive ? 'bg-green-500/10 text-green-500 hover:bg-green-500/20' : 'bg-fg/10 text-muted hover:bg-fg/20'}`}>
                          {isActive ? <Eye size={12} /> : <EyeOff size={12} />} {isActive ? 'Видима' : 'Прихована'}
                        </button>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openModal(cat)} className="p-1.5 text-muted hover:text-blue-500 rounded transition-colors"><Edit size={16} /></button>
                          <button onClick={() => setCategoryToDelete(cat._id || cat.id)} className="p-1.5 text-muted hover:text-red-500 rounded transition-colors"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-bg/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-bg border border-border rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-border bg-fg/[0.02]">
              ...
              <button onClick={() => setModalForm(null)} className="p-1 text-muted hover:text-fg rounded"><X size={20} /></button>
            </div>
            <form onSubmit={handleSave} className="p-5 space-y-4">
              ...
              {(modalForm._id || modalForm.id) && (
                <label className="flex items-center gap-2 cursor-pointer mt-2 w-fit">
                  <input type="checkbox" checked={modalForm.isActive !== undefined ? modalForm.isActive : true} onChange={(e) => setModalForm({ ...modalForm, isActive: e.target.checked })} className="rounded border-border text-industrial-accent focus:ring-industrial-accent bg-transparent" />
                  <span className="text-sm text-fg">Видима для користувачів</span>
                </label>
              )}
              <div className="pt-4 flex justify-end gap-2 border-t border-border mt-6">
                <button type="button" onClick={() => setModalForm(null)} className="px-4 py-2 text-sm font-medium text-muted hover:text-fg transition-colors">Скасувати</button>
                <button type="submit" disabled={isSaving || !modalForm.name.trim() || !modalForm.slug.trim()} className="px-4 py-2 text-sm font-medium bg-fg text-bg rounded-xl hover:opacity-90 transition-opacity flex items-center gap-2 disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-bg/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-bg border border-border rounded-2xl shadow-2xl w-full max-w-[320px] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex border-t border-border bg-fg/[0.02]">
              <button onClick={() => setCategoryToDelete(null)} disabled={isDeleting} className="flex-1 py-3 text-sm font-medium text-fg hover:bg-fg/5 border-r border-border disabled:opacity-50">Скасувати</button>
